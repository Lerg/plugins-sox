...
namespace utils {
	...
	void table_get_table(lua_State *L, const char *key) {
	...
	void table_get_string(lua_State *L, const char *key, char **value, const char *default_value) {
		if (default_value != NULL) {
			...
			luaL_error(L, "table_get_string: default value is NULL for key %s", key);
		}
	}
	...
	void add_task(int lua_listener, Event *event) {
	...
	void execute_tasks(lua_State *L) {
		while (!tasks.empty()) {
			...
			dispatch_event(L, script_listener.lua_listener, event);
			delete []event->name;
			delete []event->phase;
